...
const ScheduleManagement = () => {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            const url = isEditing
                ? `http://88.198.112.156:8000/api/schedules/${editingScheduleId}`
                : 'http://88.198.112.156:8000/api/schedules';
    ...
    const getAllSchedules = async () => {
        try {
            const response = await fetch(`http://88.198.112.156:8000/api/schedules?page=${currentPage}&limit=${limit}`);
            const responseBody = await response.json();
            return responseBody;
        } catch (error) {
            console.log("Error: ", error);
    ...
    const deleteSchedule = async (scheduleId) => {
        try {
            const response = await fetch(`http://88.198.112.156:8000/api/schedules/${scheduleId}`, {
                method: 'DELETE',
            });
