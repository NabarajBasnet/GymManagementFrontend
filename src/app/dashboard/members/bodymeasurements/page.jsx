...
const BodyMeasurements = () => {
    ...
    const getBodyMeasurementDetails = async () => {
        ...
        try {
            const response = await fetch(`http://88.198.112.156:8000/api/member/bodymeasurements/${selectedMemberId}?page=${currentPage}&limit=${limit}`);
            const responseBody = await response.json();
            if (response.ok) {
                setRenderMainContents(true);
    ...
    return (
        <div className="body-measurement-tracker min-h-screen bg-gray-100 w-full px-4 py-6">
            ...
            <div className="w-full bg-white px-4 py-1 rounded-md shadow-sm border">
                <div className="flex flex-col rounded-sm md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                    <div>
            ...
            {renderMainContents && (
                <div>
                    <Tabs defaultValue="table" className="mt-4">
                        <div className="flex py-1 px-1 rounded-md justify-between items-center mb-4">
                            <TabsList className='bg-blue-600'>
                                <TabsTrigger value="table" className='text-white'>Data Table</TabsTrigger>
