...
const MemberDetails = ({ memberId }) => {
    ...
    const holdMembership = async () => {
        ...
        try {
            const response = await fetch(`http://https://gymmanagementbackend-o2l3.onrender.com:3000/api/members/hold-membership/${memberId}`, {
                method: "PATCH",
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    const getAactionTakers = async () => {
        try {
            const response = await fetch(`http://https://gymmanagementbackend-o2l3.onrender.com:3000/api/staffsmanagement/actiontakers?actionTakers=${['Gym Admin', 'Super Admin', 'Operational Manager', 'HR Manager', 'CEO', 'Intern', 'Floor Trainer', 'Personal Trainer']}`);
            const responseBody = await response.json();
            return responseBody;
        } catch (error) {
            console.log("Error: ", error);
