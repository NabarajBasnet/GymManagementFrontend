...
const InactiveMembers = () => {
    ...
    const getAllMembers = async ({ queryKey }) => {
        ...
        try {
            const response = await fetch(
                `http://88.198.112.156:3100/api/org-members/inactive-by-branch?page=${page}&limit=${limit}&memberSearchQuery=${searchQuery}&sortBy=${sortBy}&sortOrderDesc=${sortOrderDesc}`
            );
    ...
    const sendQrInEmail = async (id) => {
        ...
        try {
            const response = await fetch(`http://88.198.112.156:3100/api/send-qr`, {
                method: "POST",
                headers: {
    ...
    const deleteMember = async (id) => {
        setIsDeleting(true);
        try {
            const response = await fetch(`http://88.198.112.156:3100/api/members/deleteMember/${id}`, {
                method: "DELETE",
                headers: {
                    'Content-Type': "application/json"
                },
            ...
            <div className='w-full px-4 pt-6 pb-3' onClick={() => {
                ...
                <Breadcrumb className='mb-6 mt-1'>
                    <BreadcrumbList>
                        ...
                        <BreadcrumbSeparator className='font-medium text-sm' />
                        <BreadcrumbItem>
                            <BreadcrumbLink className='font-medium text-sm'>Inactive Members</BreadcrumbLink>
                        </BreadcrumbItem>
                    </BreadcrumbList>
                </Breadcrumb>
                <div className="bg-white dark:bg-gray-800 shadow-sm rounded-sm border dark:border-none px-4 py-5">
                    <h1 className="text-xl font-bold dark:text-gray-300">Inactive Members</h1>
                </div>
            </div>
            ...
            <div className="mx-4 bg-white dark:bg-gray-800 shadow-lg rounded-sm dark:border-none border">
                ...
                <div className="w-full px-4 py-4 md:flex items-center justify-between">
                    <p className="font-medium text-center text-sm font-gray-700 dark:text-gray-100">
                        ...
                        of <span className="font-semibold">{totalInactiveMembers}</span> entries
                    </p>
