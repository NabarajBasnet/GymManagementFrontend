...
const PaymentDetails = () => {
    ...
    const getPaymentDetails = async ({ queryKey }) => {
        ...
        try {
            const response = await fetch(`http://88.198.115.156:3000/api/paymentdetails/${memberId}?page=${page}&limit=${limit}`);
            const responseBody = await response.json();
            return responseBody;
        } catch (error) {
            console.log("Error: ", error);
    ...
    const deletePaymentDetail = async (id) => {
        setIsDeleting(true);
        try {
            const response = await fetch(`http://88.198.115.156:3000/api/paymentdetails/${id}`,
                {
                    method: "DELETE",
                });
            const responseBody = await response.json();
