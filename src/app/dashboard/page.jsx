...
const AdminDashboard = () => {
  ...
  const getTotalMembers = async () => {
    try {
      const response = await fetch(
        `http://88.198.115.156:3000/api/members?startDate=${startDate}&endDate=${endDate}&limit=${limit}&page=${currentPage}`
      );
  ...
  const getAverageActiveMembers = async () => {
    try {
      const response = await fetch('http://88.198.115.156:3000/api/averageactivemembers');
      const responseBody = await response.json();
      if (response.ok) {
