...
const StaffTaskManagement = () => {
    ...
    const getAllTasks = async ({ queryKey }) => {
        ...
        try {
            const response = await fetch(`http://88.198.112.156:3000/api/tasks?page=${page}&limit=${limit}&taskSearchQuery=${searchQuery}&status=${status}&priority=${priority}&category=${category}`);
            const responseBody = await response.json();
            return responseBody;
        } catch (error) {
            console.log("Error: ", error);
    ...
    const getSingleTask = async (id) => {
        try {
            const response = await fetch(`http://88.198.112.156:3000/api/tasks/${id}`);
            const responseBody = await response.json();
    ...
    const addNewTask = async (data) => {
        try {
            ...
            const response = await fetch(`http://88.198.112.156:3000/api/tasks`, {
                method: "POST",
                headers: {
    ...
    const deleteSingleTask = async (id) => {
        setDeleting(true);
        try {
            const response = await fetch(`http://88.198.112.156:3000/api/tasks/delete/${id}`, {
                method: "DELETE",
            });
    ...
    const deleteSelectedTask = async () => {
        setDeleting(true);
        try {
            const response = await fetch(`http://88.198.112.156:3000/api/tasks/delete-multiple-tasks`, {
                method: "DELETE",
                headers: {
                    "Content-Type": "application/json"
                },
                                                        ...
                                                        <div className="h-8 w-8 rounded-full bg-blue-100 flex items-center justify-center">
                                                            {(() => {
                                                                ...
                                                                return assignedStaff ? (
                                                                    <img
                                                                        src={`http://http://88.198.112.156:3000:5000${assignedStaff.imageUrl}`}
