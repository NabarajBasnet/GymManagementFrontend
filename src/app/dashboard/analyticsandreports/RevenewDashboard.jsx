...
const RevenewDashboard = () => {
    ...
    const getNewMembersRevenewData = async ({ queryKey }) => {
        ...
        try {
            const response = await fetch(`https://fitbinary.com/api/revenew/new-admission-revenue?startDate=${startDate}&endDate=${endDate}&page=${currentPage}&limit=${limit}`);
    ...
    const getRenewedMembersRevenewData = async ({ queryKey }) => {
        const [, startDate, endDate, currentPage, limit] = queryKey;
        try {
            const response = await fetch(`https://fitbinary.com/api/revenew/renewd-members?startDate=${startDate}&endDate=${endDate}&page=${currentPage}&limit=${limit}`);
            const resBody = await response.json();
            return resBody;
        } catch (error) {
            console.log("Error: ", error);
    ...
    // @GET Get revenue data generated by lockers
    const getLockersRevenueData = async ({ queryKey }) => {
        const [, startDate, endDate, currentPage, limit] = queryKey;
        try {
            const response = await fetch(`https://fitbinary.com/api/revenew/revenue-lockers?startDate=${startDate}&endDate=${endDate}&page=${currentPage}&limit=${limit}`);
            const resBody = await response.json();
            return resBody;
        } catch (error) {
            console.log("Error: ", error);
    ...
    const { data: revenueByLockers, isLoading: isLockersLoading } = useQuery({
        queryKey: ['revenewbylockers', startDate, endDate, currentPage, limit],
        queryFn: getLockersRevenueData
    });

    const { lockers, totalLockers, totalPages: totalLockerPages, totalRevenue } = revenueByLockers || {};
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="flex items-baseline space-x-2">
                                        <span className="text-2xl font-bold bg-gradient-to-r from-orange-600 to-yellow-600 dark:from-orange-400 dark:to-yellow-400 bg-clip-text text-transparent">
                                            {loggedInUser?.organization?.currency || 'N/A'}{" "}
                                            {(
                                                (totalRenewMemberRevenue || 0) +
                                                (totalNewMemberRevenue || 0) +
                                                (totalRevenue || 0)
                                            ).toLocaleString()}
                                        </span>
                                        {/* <ArrowUpRight className="w-4 h-4 text-green-500" /> */}
