...
const PersonalTrainingBooking = () => {
  ...
  const getTrainingDetails = async (id) => {
    try {
      const response = await fetch(`http://88.198.112.156:8000/api/personaltraining/${id}`);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const url = isEditMode
        ? `http://88.198.112.156:8000/api/personaltraining/${editId}`
        : 'http://88.198.112.156:8000/api/personaltraining';
  ...
  const getAllPackages = async () => {
    try {
      const response = await fetch(`http://88.198.112.156:8000/api/personaltraining/packages`);
      const responseBody = await response.json();
      return responseBody;
    } catch (error) {
      console.log("Error: ", error);
  ...
  const getAllPersonalTrainingBookings = async ({ queryKey }) => {
    ...
    try {
      const response = await fetch(`http://88.198.112.156:8000/api/personaltraining?page=${page}&limit=${limit}&status=${status}&paymentStatus=${paymentStatus}&search=${search}&sortBy=${sortBy}&sortOrderDesc=${sortOrderDesc}`);
      const responseBody = await response.json();
      return responseBody;
    } catch (error) {
      console.log("Error: ", error);
