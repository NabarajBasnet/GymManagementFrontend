...
const ClassBooking = () => {
    ...
    const getAllSchedules = async () => {
        try {
            const response = await fetch(`http://88.198.112.156:8000/api/schedules?search=${debouncedSearchQuery}&category=${selectedCategory}&page=${currentPage}&limit=${limit}`);
    ...
    const handleBookClass = async (scheduleId) => {
        setIsBooking(true);
        try {
            const response = await fetch(`http://88.198.112.156:8000/api/schedules/${scheduleId}/book`, {
                method: 'PATCH',
                headers: {
