...
const MemberBodyMeasurements = () => {
    ...
    const submitBodyMeasurements = async (data) => {
        try {
            ...
            const response = await fetch(`http://88.198.112.156:3100/api/member/bodymeasurements/${memberId}`, {
                method: "POST",
                headers: {
    ...
    const getAllBodyMeasurements = async ({ queryKey }) => {
        const [, page] = queryKey;
        try {
            const response = await fetch(`http://88.198.112.156:3100/api/member/bodymeasurements/${memberId}?page=${page}&limit=${limit}`);
            const responseBody = await response.json();
            return responseBody;
        } catch (error) {
            console.log('Error', error);
    ...
    const handleDelete = async (id) => {
        try {
            const response = await fetch(`http://88.198.112.156:3100/api/member/bodymeasurements/${id}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json'
                },
