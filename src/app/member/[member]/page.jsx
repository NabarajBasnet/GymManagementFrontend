...
function MemberPortal() {
    ...
    return (
        <>
            ...
            {/* Sidebar */}
            <div className={`${isMobileMenuOpen ? 'block' : 'hidden'} md:block md:w-64 bg-white border-r border-gray-200 shadow-sm`}>
                <div className="flex flex-col h-full">
                    ...
                    <nav className="flex-1 px-2 py-4">
                        ...
                        <SidebarLink
                            ...
                            onClick={() => { setActiveTab('measurements'); setIsMobileMenuOpen(false); }}
                        />
                        <SidebarLink
...
function QRCodeSection({ memberData }) {
    return (
...
function ChatSection({ trainers, selectedTrainer, setSelectedTrainer, messages, chatInput, setChatInput, handleSendMessage }) {
    return (
        <div>
            <h2 className="text-2xl font-bold mb-6">Chat with Staff</h2>

            <div className="bg-white rounded-lg shadow-md flex flex-col md:flex-row h-96 md:h-[32rem]">
                {/* Trainers list */}
                <div className="w-full md:w-64 border-r border-gray-200 overflow-y-auto">
                    <div className="p-4 border-b border-gray-200">
                        <h3 className="font-medium">Trainers & Staff</h3>
                    </div>
                    <div>
                        {trainers.map(trainer => (
                            <button
                                ...
                                className={`flex items-center w-full p-4 border-b border-gray-100 hover:bg-gray-50 ${selectedTrainer.id === trainer.id ? 'bg-indigo-50' : ''
                                    }`}
                            >
                                <div className="relative">
                                    <img src={trainer.avatar} alt={trainer.name} className="w-10 h-10 rounded-full" />
                                    <span className={`absolute bottom-0 right-0 block w-3 h-3 rounded-full ${trainer.status === 'online' ? 'bg-green-400' : 'bg-gray-400'
                                        } border-2 border-white`}></span>
                                </div>
                                <div className="ml-3">
                                    <p className="font-medium">{trainer.name}</p>
                                    <p className="text-xs text-gray-500 capitalize">{trainer.status}</p>
                                </div>
                            </button>
                        ))}
                    </div>
                ...
                <div className="flex-1 flex flex-col">
                    ...
                    <div className="p-4 border-b border-gray-200 flex items-center">
                        <div className="relative">
                            <img src={selectedTrainer.avatar} alt={selectedTrainer.name} className="w-8 h-8 rounded-full" />
                            <span className={`absolute bottom-0 right-0 block w-2 h-2 rounded-full ${selectedTrainer.status === 'online' ? 'bg-green-400' : 'bg-gray-400'
                                } border-2 border-white`}></span>
                        </div>
                        <div className="ml-3">
...
function FeedbackSection({ trainers }) {
    return (
        <div>
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
                <form>
                    ...
                    <div className="mb-4">
                        <label htmlFor="trainer" className="block text-gray-700 font-medium mb-2">
